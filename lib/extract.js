        ...
        var extractComments = function (comments, src) {
            ...
            var handleExportNamedDeclaration = function (row, line) {
                ...
                row.declaration.body.body.forEach(function (declaration) {
                    if (declaration.loc.start.line <= line && declaration.loc.end.line >= line) {
                        if (declaration.type === 'MethodDefinition') {
                            declaration.value.body.body.forEach(function (expression) {
                                if (expression.type === 'ExpressionStatement' && expression.loc.start.line <= line && expression.loc.end.line >= line) {
                                    if (expression.expression.type === 'AssignmentExpression' && !expression.expression.right.hasOwnProperty('properties')) {
                                        comment = getJSExpression(expression.expression);
                                    } else {
                                        expression.expression.right.properties.forEach(function (property) {
                                            if (property.loc.start.line === line) {
                                                comment = handleProperty(property, line);
                                            }
                                        });
                                    }
                                } else if (expression.type === 'VariableDeclaration' && expression.loc.start.line <= line && expression.loc.end.line >= line) {
                                    comment = handleVariableDeclaration(expression, line);
                                }
                            });
            ...
            var extractNodeComment = function (loc) {
                ...
                src.forEach(function (row) {
                    if (row.loc.start.line <= line && row.loc.end.line >= line) {
