...
function getJSExpression(node) {
    ...
    if (node.type === 'AssignmentExpression' && isStringLiteral(node.right)) {
...
var Extractor = (function () {
    function Extractor(options) {
        this.options = _.extend({
            ...
            moduleMethodObservable: 'getObservable',
            moduleMethodPlural: 'getPlural',
            moduleComment: 'capture-string-as-translation',
            attribute: 'translate',
            attributes: [],
            lineNumbers: true,
            extensions: {
    ...
    Extractor.prototype.extractJs = function (filename, src, lineNumber) {
        ...
        function isGetString(node) {
            return node !== null &&
                ...
                (node.callee.property.name === self.options.moduleMethodString ||
                    node.callee.property.name === self.options.moduleMethodObservable) &&
                node.arguments !== null &&
                node.arguments.length;
        }
        ...
        walkJs(syntax, function (node, parentComment) {
            ...
            if (isGettext(node) || isGetString(node)) {
                ...
                self.extractComments(reference.file, node.comments, node.body);
            }
            if (str || singular) {
    ...
    Extractor.prototype.extractComments = function (filename, comments, src) {
        var extractComments = function (comments, src) {
            var self = this;

            var reference = function (lineNumber) {
                return {
                    file: filename,
                    location: {
                        start: {
            ...
            var checkRow = function (row, line) {
                return row.loc.start.line <= line && row.loc.end.line >= line;
            };

            var handleProperty = function (property, line) {
                var comment = '';
                if (property.type === 'Property' && property.loc.start.line === line) {
                    comment = getJSExpression(property.value);
                }
                return comment;
            };

            var handleVariableDeclaration = function (row, line) {
                var comment = '';
                row.declarations.forEach(function (declaration) {
                    if (declaration.type === 'VariableDeclarator') {
                        if (declaration.init.hasOwnProperty('properties')) {
                            declaration.init.properties.forEach(function (property) {
                                if (property.loc.start.line === line) {
                                    comment = handleProperty(property, line);
                                }
                            });
                        } else {
                            comment = getJSExpression(declaration.init);
                        }
                    }
                });
                return comment;
            ...
            var handleExpressionStatement = function (row, line) {
                var comment = '';
                if (row.expression.type === 'AssignmentExpression' && !row.expression.right.hasOwnProperty('properties')) {
                    comment = getJSExpression(row.expression);
                } else {
                    row.expression.right.properties.forEach(function (property) {
                        if (property.loc.start.line === line) {
                            comment = handleProperty(property, line);
                        }
                    });
            ...
            var extractJSComment = function (row, line) {
                var comment = '';
                if (row.type === 'ExpressionStatement') {
                    comment = handleExpressionStatement(row, line);
                } else if (row.type === 'VariableDeclaration') {
                    comment = handleVariableDeclaration(row);
                } else if (row.type === 'IfStatement') {
                    comment = handleIfStatement(row, line);
                } else if (row.type === 'SwitchStatement') {
                    comment = handleSwitchCaseStatement(row, line);
                }
                return comment;
            };

            var handleBlockStatement = function (statement, line) {
                var comment = '';
                statement.body.forEach(function (declaration) {
                    if (checkRow(declaration, line)) {
                        comment = extractJSComment(declaration, line);
                    }
                });
                return comment;
            };

            var handleIfStatement = function (row, line) {
                var comment = '';

                if (row.type === 'BlockStatement') {
                    comment = handleBlockStatement(row, line);
                    return comment;
                }

                if (checkRow(row.consequent, line)) {
                    row.consequent.body.forEach(function (declaration) {
                        if (checkRow(declaration, line)) {
                            comment = extractJSComment(declaration, line);
                        }
                    });
                } else {
                    comment = handleIfStatement(row.alternate, line);
                }
                return comment;
            };

            var handleSwitchCaseStatement = function (row, line) {
                var comment = '';
                if (checkRow(row, line)) {
                    row.cases.forEach(function (c) {
                        if (checkRow(c, line)) {
                            c.consequent.forEach(function (cons) {
                                if (checkRow(cons, line)) {
                                    comment = extractJSComment(cons, line);
                                }
                            });
                        }
                    });
            ...
            var handleExportNamedDeclaration = function (row, line) {
                var comment = '';
                row.declaration.body.body.forEach(function (declaration) {
                    if (checkRow(declaration, line)) {
                        if (declaration.type === 'MethodDefinition') {
                            declaration.value.body.body.forEach(function (expression) {
                                if (!checkRow(expression, line)) {
                                    return;
                                }
                                comment = extractJSComment(expression, line);
                            });
                        } else if (declaration.type === 'ClassProperty') {
                            if (declaration.value.hasOwnProperty('properties')) {
                                declaration.value.properties.forEach(function (property) {
                                    if (property.loc.start.line === line) {
                                        comment = handleProperty(property, line);
                                    }
                                });
                            } else {
                                comment = getJSExpression(declaration.value, line);
                            }
                        }
                    }
                });
            ...
            var extractNodeComment = function (loc) {
                var commentInner = '';
                var line = loc.start.line;
                src.forEach(function (row) {
                    if (checkRow(row, line)) {
                        if (row.type === 'VariableDeclaration') {
                            commentInner = handleVariableDeclaration(row, line);
                        } else if (row.type === 'ExportNamedDeclaration') {
                            commentInner = handleExportNamedDeclaration(row, line);
                        }
                    }
                });
                return commentInner;
            ...
            comments.forEach(function (comment) {
                if (comment.type === 'Line' && comment.value.includes(self.options.moduleComment)) {
                    var text = extractNodeComment(comment.loc);
                    if (text) {
                        self.addString(reference(comment.loc.start.line), text);
                    }
                }
            });
        }.bind(this);
